...
class HtmlInjector {
    ...
    getInjectData() {
        return {
            ...
            OG_SITE_NAME: this.config.og?.siteName || 'Collab',
            OG_TITLE: this.config.og?.title || 'Click the link to make a call.',
            OG_DESCRIPTION:
                this.config.og?.description ||
                'Collab calling provides real-time video calls, messaging and screen sharing.',
            OG_IMAGE: this.config.og?.image || 'https://sfu.collab.com/images/collab.png',
            OG_URL: this.config.og?.url || 'https://sfu.collab.com',
